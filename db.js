...
//class Methods
Puppy.findByFavFood = function(food) {
  return this.findAll({
         where: {
           favFood: food
         }
       }).then(function(puppiesWithFavFood){
          console.log('pups that like ' + food + ' ' + Object.keys(puppiesWithFavFood).length)
       })
}

Puppy.count = function() {
  this.findAll()
               .then(function(puppies) {
                 console.log('number of pups: ' + puppies.length)
                 return puppies.length;
               })
}
